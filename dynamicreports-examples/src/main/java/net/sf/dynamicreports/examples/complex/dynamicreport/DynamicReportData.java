...
public class DynamicReportData implements ReportData {
	...
	public DynamicReport getUserReport() {
		DynamicReport report = new DynamicReport();

    Map<String, TextColumnBuilder> columns = new LinkedHashMap<String, TextColumnBuilder>();
    columns.put("number", col.reportRowNumberColumn("No."));
    columns.put("state",col.column("State", "state", type.stringType()));
    columns.put("item",col.column("Item", "item", type.stringType()));
    columns.put("orderdate", col.column("Order date", "orderdate", type.dateType()));
    columns.put("quantity", col.column("Quantity", "quantity", type.integerType()));
    columns.put("unitprice", col.column("Unit price", "unitprice", type.bigDecimalType()));
    ...
    subtotals.add("quantity");
    subtotals.add("unitprice");
    report.setSubtotals(subtotals);

    return report;
	}

	public class DynamicReport {
		private Map<String, TextColumnBuilder> columns;
		private List<String> groups;
		private List<String> subtotals;

		public Map<String, TextColumnBuilder> getColumns() {
			return columns;
		}

		public void setColumns(Map<String, TextColumnBuilder> columns) {
